...
const ongoingScheduleFormSchema = z.object({
  title: z
    .string()
    .trim()
...
type OngoingScheduleFormData = z.infer<typeof ongoingScheduleFormSchema>

interface OngoingScheduleFormProps {
  mode: 'create' | 'edit'
  schedule?: Schedule
  onSuccess: () => void
  onCancel: () => void
...
export function OngoingScheduleForm({ mode, schedule, onSuccess, onCancel }: OngoingScheduleFormProps) {
  ...
  const form = useForm<OngoingScheduleFormData>({
    resolver: zodResolver(ongoingScheduleFormSchema),
    defaultValues: {
      title: '',
      description: '',
  ...
  const onSubmit = async (data: OngoingScheduleFormData) => {
    try {
      // Mock API call - replace with actual API integration later
      console.log('Ongoing Schedule data:', data)
      
      // Simulate API delay
      await new Promise(resolve => setTimeout(resolve, 1000))
      
      toast({
        title: mode === 'create' ? 'Ongoing Meeting Created' : 'Ongoing Meeting Updated',
      ...
      toast({
        ...
        description: error?.message || `Failed to ${mode} ongoing meeting.`,
        variant: 'destructive',
      })
    }
  }
  ...
  // Get date/time constraints for ongoing meetings
  const getDateConstraints = () => {
    ...
    return {
      ...
      minTime: '00:00',
      maxTime: '23:59',
      placeholder: 'Select today\'s date and time'
    }
  }
          ...
          <FormField
            ...
            render={({ field }) => (
              <FormItem className="md:col-span-2">
                ...
                <FormControl>
                  <Input 
                    placeholder="Ongoing Meeting Title" 
                    {...field} 
                    disabled={isLoading}
                  />
                </FormControl>
          ...
          <FormField
            ...
            render={({ field }) => (
              <FormItem className="md:col-span-2">
                ...
                <FormControl>
                  <Textarea 
                    placeholder="Describe the purpose and agenda of this ongoing meeting..."
        ...
        <div className="flex justify-end space-x-2">
          ...
          >
            {isLoading 
              ...
              : (mode === 'create' ? 'Create Ongoing Meeting' : 'Update Ongoing Meeting')
            }
          </Button>
        </div>
      </form>
