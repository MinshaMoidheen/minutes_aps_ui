...
// Mock data for ongoing meetings (today's date)
const mockOngoingSchedules: Schedule[] = [
  {
    _id: '1',
...
export function OngoingScheduleList({ onEdit }: OngoingScheduleListProps) {
  const router = useRouter()
  const [searchTerm, setSearchTerm] = useState('')
  const [schedules] = useState<Schedule[]>(mockOngoingSchedules)
  ...
  const handleComplete = (schedule: Schedule) => {
    // Mock complete functionality
    toast({
      title: 'Meeting Completed',
      description: `${schedule.title} has been marked as completed.`,
    })
  }
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center space-x-2">
        <div className="relative flex-1 max-w-sm">
          ...
          <Input
            placeholder="Search ongoing meetings..."
            value={searchTerm}
        ...
        <Badge variant="default" className="ml-auto">
          {filteredSchedules.length} ongoing meeting{filteredSchedules.length !== 1 ? 's' : ''}
        </Badge>
      </div>
            ...
            {filteredSchedules.length === 0 ? (
              <TableRow>
                <TableCell colSpan={8} className="h-24 text-center">
                  {searchTerm ? 'No ongoing meetings found matching your search.' : 'No ongoing meetings found.'}
                </TableCell>
              </TableRow>
            ) : (
                  ...
                  <TableCell>
                    <DropdownMenu>
                      ...
                      <DropdownMenuContent align="end">
                        ...
                        <DropdownMenuItem onClick={() => handleComplete(schedule)}>
      ...
      <div className="flex items-center justify-between">
        <p className="text-sm text-muted-foreground">
          Showing {filteredSchedules.length} of {schedules.length} ongoing meetings
        </p>
        <div className="flex items-center space-x-2">
