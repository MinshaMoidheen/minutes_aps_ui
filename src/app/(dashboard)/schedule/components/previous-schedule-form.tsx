...
const previousScheduleFormSchema = z.object({
  title: z
    .string()
    .trim()
...
type PreviousScheduleFormData = z.infer<typeof previousScheduleFormSchema>

interface PreviousScheduleFormProps {
  mode: 'create' | 'edit'
  schedule?: Schedule
  onSuccess: () => void
  onCancel: () => void
...
export function PreviousScheduleForm({ mode, schedule, onSuccess, onCancel }: PreviousScheduleFormProps) {
  ...
  const form = useForm<PreviousScheduleFormData>({
    resolver: zodResolver(previousScheduleFormSchema),
    defaultValues: {
      title: '',
      description: '',
      startDate: '',
  ...
  const onSubmit = async (data: PreviousScheduleFormData) => {
    try {
      // Mock API call - replace with actual API integration later
      console.log('Previous Schedule data:', data)
      
      // Simulate API delay
      await new Promise(resolve => setTimeout(resolve, 1000))
      
      toast({
        title: mode === 'create' ? 'Previous Meeting Created' : 'Previous Meeting Updated',
      ...
      toast({
        ...
        description: error?.message || `Failed to ${mode} previous meeting.`,
        variant: 'destructive',
      })
    }
  }
  ...
  // Get date/time constraints for previous meetings
  const getDateConstraints = () => {
    const today = new Date().toISOString().split('T')[0]
    
    return {
      maxDate: new Date(Date.now() - 24 * 60 * 60 * 1000).toISOString().split('T')[0], // Yesterday
          ...
          <FormField
            ...
            render={({ field }) => (
              <FormItem className="md:col-span-2">
                ...
                <FormControl>
                  <Input 
                    placeholder="Previous Meeting Title" 
                    {...field} 
                    disabled={isLoading}
                  />
                </FormControl>
          ...
          <FormField
            ...
            render={({ field }) => (
              <FormItem className="md:col-span-2">
                ...
                <FormControl>
                  <Textarea 
                    placeholder="Describe the purpose and agenda of this previous meeting..."
        ...
        <div className="flex justify-end space-x-2">
          ...
          >
            {isLoading 
              ...
              : (mode === 'create' ? 'Create Previous Meeting' : 'Update Previous Meeting')
            }
          </Button>
        </div>
      </form>
